...
// Image compression - compromis taille/qualite pour limiter le flou
// 1200px + qualite 0.6 : plus net que 800/0.4, tout en restant compresse
...
export default function ImagesAvantPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Build a quick lookup { "title|idx": { description, urls[], is_compressed } }
      const map = {};
    ...
    try {
      ...
      if (compressedImages > 0) {
        ...
        const res = await fetch('/api/images', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ projectId: Number(id), section: 'avant', entries: updatedEntries }),
        });
